...
def mrr_inference(adaptor_save_dir, infer_file_path, output_infer_file):
    print("run mrr inference")
    # 配置
    batch_size = 16

    # adaptor_save_dir = "../save_model/go/go_adaptor"
    fine_turn_model_name = "../../java_fine_turn_GraBertlastepoch_lookLoss_GraBert"
    ...
    #load fusion adaptor
    model.load_adapter(adaptor_save_dir + "/go")
    model.load_adapter(adaptor_save_dir + "/python")
    model.load_adapter(adaptor_save_dir + "/java")

    model.load_adapter_fusion(adaptor_save_dir, set_active=True)
...
def infer_specific_lang():
    lang = ['python']
    model_suffix = "_fusion"
    for l in lang:
        adaptor_save_dir = "../../save_model/" + l + "/" + l + model_suffix
        infer_file_path = "../../data/test/"+ l +"/batch_0.txt"
        output_infer_file = "../../results/"+ l +"/adaptor_batch_0.txt"
        mrr_inference(adaptor_save_dir, infer_file_path, output_infer_file)

if __name__ == '__main__':

    # adaptor_save_dir = "../../save_model/ruby/ruby_fusion_two"  # arg3
    # infer_file_path = "../../data/test/ruby/batch_0.txt"
    # output_infer_file = "../../results/ruby/adaptor_batch_0.txt"
    # mrr_inference(adaptor_save_dir, infer_file_path, output_infer_file)
    infer_specific_lang()