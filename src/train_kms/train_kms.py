...
import torch
import os
from transformers import AutoModelForSequenceClassification, get_linear_schedule_with_warmup, AdapterConfig
...
from src.make_hard_samples.a1_hard_samples_by_faiss import read_data, write_data_to_file
...
def look_and_write_nearst_files(in_file_path, output_file, nearest_text):
    ...
    for text, code, near_text_index in zip(text_lines, code_lines, nearest_text):
        ...
        if (str(text) != str(text_lines[near_text_index])):  # 防止query被同时作正例和反例
            new_all_quries.append(texadaptor_t_lines[near_text_index])
        else:
...
def main(num_epochs, batch_size, lang, train_file_path, infer_file_path, output_infer_file):
    set_seed(1)
    print("run")
    ...
    tokenizer = AutoTokenizer.from_pretrained("microsoft/graphcodebert-base") #换上solidity_bpe_tokenizer，降低幅度很大，为什么呢？
    ...
    for epoch in range(num_epochs):
        ...
        for text, code, labels in train_dataLoader:
            ...
            scaler.update()
            scheduler.step() # no use scheduler
            optimizer.zero_grad()
...
if __name__ == '__main__':
    ...
    # train file
    kms_suffix_name = "/" + lang + "_adaptor_onceAllFile_2e4" #_adaptor_onceAllFile_2e4

    adaptor_save_dir = "../../save_model/" + lang + kms_suffix_name  # arg3
    train_file_path = "../../data/train_valid/" + lang + "/train.txt"

    # valid file
    infer_file_path = "../../data/test/" + lang + "/batch_0.txt"
    output_infer_file = "../../results/" + lang + "/adaptor_batch_0.txt"

    main(num_epochs, batch_size, lang, train_file_path, infer_file_path, output_infer_file)
